from os import listdir, path  # noqa
import numpy as np
import scipy  # noqa
import cv2
import os
...
parser.add_argument('--face_det_batch_size', type=int, help='Batch size for face detection', default=4)  # 16
parser.add_argument('--wav2lip_batch_size', type=int, help='Batch size for Wav2Lip model(s)', default=16)  # 128
...
def load_model(ckp_path):
    model = Wav2Lip()
    print("Load checkpoint from: {}".format(ckp_path))
    checkpoint = _load(ckp_path)
